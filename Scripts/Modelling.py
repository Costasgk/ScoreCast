...
def train_model(data, predictors, url):
    ...
    train = data[data["date"] <= '2023-08-30']
    test = data[data["date"] > '2023-08-30']
    if url == url_arg:
...
if __name__ == "__main__":
    ...
    urls = [url_br_a, url_br_b, url_arg, url_jpn, url_nor, url_fin, url_eng, url_ita ]
    
    for idx, url in enumerate(urls):
        
        df_train = read_train(url)
        
        if url == url_arg or url == url_ita:
