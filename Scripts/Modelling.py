...
def train_model(data, predictors, test_days=180):
    rf = RandomForestClassifier(n_estimators=100, min_samples_split=10, random_state=42)
    # Split relative to the data, not a fixed date — a hardcoded 2023 cutoff
    # silently trains on nothing new once the CSVs move past it.
    split = data["date"].max() - pd.Timedelta(days=test_days)
